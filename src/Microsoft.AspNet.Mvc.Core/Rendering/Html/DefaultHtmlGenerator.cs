    ...
    public class DefaultHtmlGenerator : IHtmlGenerator
    {
            ...
            object htmlAttributes)
        {
            ...
            if (!string.IsNullOrEmpty(message))
            {
                ...
                wrappedMessage.Append(messageTag);
        ...
        private IHtmlContent GenerateGroupsAndOptions(string optionLabel, IEnumerable<SelectListItem> selectList)
        {
            ...
            foreach (var group in groupedSelectList)
            {
                ...
                if (optGroup == null)
                {
                    AppendOptions(listItemBuilder, group);
                }
                else
                {
                    groupBuilder.InnerHtml = new BufferedHtmlContent();
                    AppendOptions((BufferedHtmlContent)groupBuilder.InnerHtml, group);
                    listItemBuilder.Append(groupBuilder);
                    listItemBuilder.Append(Environment.NewLine);
                }
            }
        ...
        private void AppendOptions(BufferedHtmlContent list, IGrouping<int, SelectListItem> group)
        {
            foreach (var item in group)
            {
                list.Append(GenerateOption(item));
                list.Append(Environment.NewLine);
            }
        }

        private TagBuilder GenerateOption(SelectListItem item)
        {
